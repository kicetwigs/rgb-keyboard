...
class TrackDefaultNote:
    def __init__(self, track_index):
        self.track = Track(track_index)
        self.DEFAULT_NOTE = self.get_default_note()

    def get_default_note(self):
        # Define different default notes for different indices
        note_mapping = {
            0: 36,
            1: 38,
            2: 42,
        ...
        return note_mapping.get(self.track.index, 36)  # Default to 60 if index not found
...
class Track:
    ...
    def __init__(self, sequencer, index, channel, rgb):
        ...
        # Set the default note for this track
        self.default_note = self.get_default_note()

        # For each key in the track, create a Step instance and add to self.steps.
        for i, key in enumerate(self.track_keys):
            step = Step(i, key, self, self.default_note)
    ...
    def get_default_note(self):
        ...
        return note_mapping.get(self.index, 36)  # Default to 36 if index not found
...
class Step:
    ...
    """
    def __init__(self, index, key, track, default_note):
        self.index = index
        self.key = key
        self.track = track
        self.active = False
        ...
        self.note = default_note  # Use the track's default note
        self.last_notes = []
